...
import { Pass } from 'https://unpkg.com/three@0.155.0/examples/jsm/postprocessing/Pass.js';
...
// Independent spin for hero (+beads live under heroGroup so they match perfectly)
...
const HERO_BREATH = { amp: 0.6, freq: 0.2 };      // radians, Hz-ish
const HERO_RETURN_SPEED = 3.0;                    // how fast heroSpin eases back to identity when at top
const SCROLL_TOP_EPS = 0.001;                     // threshold to treat as "at top"

/* ---------------- LOADER UI ---------------- */
const LOADER = {
  height: 2, // px
  zIndex: 999999,
  gradient: ['#BCE9F2', '#EAC697', '#FEF5EE'], // 3-col gradient
  fadeOutMs: 450,
  minShowMs: 350 // keeps it visible even if assets are cached
};

function createTopLoaderBar() {
  ...
  Object.assign(wrap.style, {
    ...
    zIndex: `${LOADER.zIndex}`,
    pointerEvents: 'none',
    opacity: '1',
    transition: `opacity ${LOADER.fadeOutMs}ms ease`,
  });
  ...
  Object.assign(bar.style, {
    ...
    transform: 'scaleX(0)',
    background: `linear-gradient(90deg, ${LOADER.gradient.join(', ')})`,
    willChange: 'transform',
  });

  wrap.appendChild(bar);
  // IMPORTANT: body might not exist yet in some embeds
  ...
  function tick() {
    ...
    if (current < 1) rafId = requestAnimationFrame(tick);
    else rafId = null;
  }
  ...
  return {
    setProgress(v) {
      ...
      const wait = Math.max(0, LOADER.minShowMs - elapsed);

      setTimeout(() => {
        wrap.style.opacity = '0';
        setTimeout(() => wrap.remove(), LOADER.fadeOutMs + 50);
      }, wait);
    },
    error() {
      this.setProgress(1);
...
/* ---------------- INIT ---------------- */
// IMPORTANT: don’t run before DOM exists (common in Webflow/Shopify head embeds)
...
async function init(){
  // Loader MUST be created after (or regardless of) body existence
  const loaderBar = createTopLoaderBar();
  loaderBar.setProgress(0.06); // nudge visible immediately (even if cached)

  const canvas = document.getElementById('webgl');
  if (!canvas) {
    console.warn('[Three] #webgl canvas not found');
    loaderBar.error();
    return;
  }

  // --- loading manager (drives progress bar) ---
  const manager = new THREE.LoadingManager();
  manager.onProgress = (_url, itemsLoaded, itemsTotal) => {
    loaderBar.setProgress(itemsTotal ? (itemsLoaded / itemsTotal) : 0);
  };
  manager.onLoad  = () => loaderBar.done();
  manager.onError = () => loaderBar.error();

  // loaders using shared manager
  ...
  try {
    ...
    hdr.dispose(); pmrem.dispose(); scene.environment = envMap;
  ...
  // Keep beads directly under heroGroup so hero+beads always match
  ...
  // HERO spin wrapper (reparent hero under it)
  heroSpin = new THREE.Group();
  scene.add(heroSpin);
  heroSpin.add(heroGroup); // re-parents heroGroup

  // Bokeh spin wrapper -> actual bokeh group
  ...
  // Petal spin wrapper -> actual petal group
...
function setHeroOpacity(a){
  ...
  if (heroMaterial){
    ...
    // IMPORTANT: don't occlude beads when hero is (nearly) transparent
...
/* ---------------- LOOP ---------------- */
const Q_IDENTITY = new THREE.Quaternion(); // for snapping/slerping back to start
...
function worldArrayToLocal(obj, posArray){
  const v = new THREE.Vector3(); obj.updateMatrixWorld(true);
  for (let i=0;i<posArray.length;i+=3){ v.set(posArray[i],posArray[i+1],posArray[i+2]); obj.worldToLocal(v);
    posArray[i]=v.x; posArray[i+1]=v.y; posArray[i+2]=v.z; }
  return posArray;
}
function worldNormalsToLocal(obj, normals){
...
window.addEventListener('message', (e) => {
  const d = e?.data;
  if (d && d.type === 'wf-scroll' && typeof d.scrollY === 'number') { externalScrollY = d.scrollY; useExternalScroll = true; }
});
function getScrollY(){ return useExternalScroll ? externalScrollY : window.scrollY; }
...
async function init(){
  ...
  const [beadGeom, hero, track] = await Promise.all([
    ...
    loadGLTFScene(MODEL_GLTF),
    loadGLTFScene(TRACK_GLTF)
  ]);
...
/* ---------------- LOADERS ---------------- */
async function loadGLTFScene(url){ const gltf = await new GLTFLoader().loadAsync(url); return gltf.scene; }
async function loadBeadGeometry(p){
  const gltf = await new GLTFLoader().loadAsync(p); let geo=null;
  gltf.scene.traverse(ch=>{ if(ch.isMesh && !geo) geo = ch.geometry.clone(); });
  if(!geo) geo=new THREE.SphereGeometry(0.02,12,12);
  if(!geo.attributes.normal){ geo=geo.clone(); geo.computeVertexNormals(); }
  return geo;
...
function estimateArea(geometry){
  ...
  if(idx){
    ...
    for(let i=0;i<pos.count;i+=3){ a.fromBufferAttribute(pos, i); b.fromBufferAttribute(pos, i+1); c.fromBufferAttribute(pos, i+2); area += triangleArea(a,b,c); }
  }
  return Math.max(area, 1e-6);
}
function triangleArea(a,b,c){
...
function animate(){
  ...
  if (heroGroup && heroSpin){
    if (scrollS <= SCROLL_TOP_EPS){
      // 1) breathe on local Y (no drift)
      heroGroup.rotation.x = 0;
      heroGroup.rotation.z = 0;
      heroGroup.rotation.y = Math.sin(t * HERO_BREATH.freq) * HERO_BREATH.amp;

      // 3) ease heroSpin wrapper back to identity (starting rotation)
      const backK = 1.0 - Math.exp(-HERO_RETURN_SPEED * dt);
      heroSpin.quaternion.slerp(Q_IDENTITY, backK);
    } else {
      // 2) when scrolling, stop breathing & spin fully via wrapper
      heroGroup.rotation.y = 0;
      heroSpin.rotateOnAxis(HERO_GROUP_ROT.axis, HERO_GROUP_ROT.speed * dt);
    }
  }

  // Independent group spins (bokeh & petals keep doing their own thing)
  if (bokehSpin) bokehSpin.rotateOnAxis(BOKEH_GROUP_ROT.axis, BOKEH_GROUP_ROT.speed * dt);
  if (petalSpin) petalSpin.rotateOnAxis(PETAL_GROUP_ROT.axis, PETAL_GROUP_ROT.speed * dt);
